...
%------------------------------------------------------
lag1collpos         =max(0,x(13))^2;
lag1collneg         =max(0,-x(13))^2;
lag2collpos         =max(0,x(14))^2;
lag2collneg         =max(0,-x(14))^2;

lag1houseshortpos  =max(0,x(15))^2;
lag1houseshortneg  =max(0,-x(15))^2;
lag2houseshortpos  =max(0,x(16))^2;
lag2houseshortneg  =max(0,-x(16))^2;
%-------------------------------------------------------
% guesses for prices
%------------------------------------------------------
q1g=exp(x(17));
...
% First order conditions
%---------------------------------------------------------------------
fval(9)=q1g*MValue1today-(beta*Value1today^(1/psi1)*EValue1^((1-phi1)/phi1)*q11);
%q1 =(beta*Value1today^(1/psi1)*EValue1^((1-phi1)/phi1)*q11)/MValue1today;
fval(10)=qhg*MValue1today-(lag1houseshortpos+lag1collpos*margin1*pmin+MValueh1today+beta*Value1today^(1/psi1)*EValue1^((1-phi1)/phi1)*qh1);
fval(11)=qhg*MValue2today-(lag2houseshortpos+lag2collpos*margin1*pmin+MValueh2today+beta*Value2today^(1/psi2)*EValue2^((1-phi2)/phi2)*qh2);

fval(12)=qbg*MValue1today-(lag1collpos+beta*Value1today^(1/psi1)*EValue1^((1-phi1)/phi1)*qb1);
fval(13)=qbg*MValue2today-(lag2collpos+beta*Value2today^(1/psi2)*EValue2^((1-phi2)/phi2)*qb2);
...
%-------------------------------------------------------------------
fval(16)=lag1collneg-(hh1houseguess*margin1*pmin+hh1bondguess);
fval(17)=lag2collneg-(hh2houseguess*margin1*pmin+hh2bondguess);
fval(18)=lag1houseshortneg-hh1houseguess;
fval(19)=lag2houseshortneg-hh2houseguess;
end
