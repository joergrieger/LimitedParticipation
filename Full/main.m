...
hh1house    =ones(nendog,8)*0.5;
phouse      =ones(nendog,8)*(-200);
ch1         =ones(nendog,8);
ch2         =ones(nendog,8);
Value1      =ones(nendog,8)*0;
Value2      =ones(nendog,8)*0;
optimSolutions=ones(nendog,8,22)*(0.05);
%------------------------------
...
options = optimoptions(@fmincon,'MaxIter',5000000,'MaxFunEvals',500000000,'Display','iter','Algorithm','interior-point','SubproblemAlgorithm','ldl-factorization','FinDiffType','forward','TolCon',1e-7);
%-------------------------------------
%Load old solutions
%-------------------------------------
load('Solutions_tempa.mat');
%load('Solutions_g200_m050_p150_e160.mat');
max_error=100;
time_iter=0;
%-------------------------------
...
while max_error>1e-4
	...
	for exstate=1:8
		...
		for endstate1=1:nendog
            ...
            xguess=x;
            %endstate1=z;
			passet1n(endstate1,exstate)=x(20);
            pbondn(endstate1,exstate)=x(21);
            phousen(endstate1,exstate)=x(22);
			hh1asset1n(endstate1,exstate)=x(9);
            hh1bondn(endstate1,exstate)=x(10);
            hh1housen(endstate1,exstate)=x(11);
            ch1n(endstate1,exstate)=x(12);
            ch2n(endstate1,exstate)=x(13);
...
save('Solutions_g200_m050_p150_e100.mat','passet1','pbond','phouse','hh1asset1','hh1bond','hh1house','ch1','ch2','Value1','Value2','optimSolutions','max_error');
